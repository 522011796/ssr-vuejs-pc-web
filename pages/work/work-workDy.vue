      ...
      <Row :gutter="16">
        <Col span="8" v-for="(item,index) in item.data" :key="index" style="margin-bottom: 10px">
          <div class="listClass" @click.prevent.self="openDetail(item)">
            ...
            <div class="listContent" @click="openDetail(item)">
              <div class="listTitle">
                <label v-if="$utils.getLength(item.title) > 20" :title="item.title">{{item.title.substring(0,15)+"......"}}</label>
